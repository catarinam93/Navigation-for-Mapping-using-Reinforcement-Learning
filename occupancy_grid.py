...
class OccupancyGrid(ABC):
    def __init__(self, origin: (float, float), dimensions: (int, int), resolution: float):
        ...
        self.max_coords: (float, float) = tuple(self.origin[i] + self.resolution * self.dimensions[i] for i in [0, 1])
        # maximum real (x,y) coordinates inside the grid (these values are not included in the grid)
